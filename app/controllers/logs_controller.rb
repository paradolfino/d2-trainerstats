class LogsController < ApplicationController
    ...
    def create
        ...
        if @log.save
            create_event("created", "log(Title: #{@log.title}, Active: #{@log.active})")
            redirect_to @log
        else
            render 'new'
        end
    ...
    def update
        ...
        if @log.update(params_log)
            create_event("updated", "log(Title: #{@old_title}) to (Title: #{@log.title}, Active: #{@log.active})")
    ...
    def destroy
        create_event("destroyed", "log(Title: #{@log.title}, Active: #{@log.active})")
        @log.destroy
        redirect_to logs_path
