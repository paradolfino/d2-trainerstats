class LogsController < ApplicationController
    ...
    def create
        ...
        if @log.save
            @event = Event.create(action: "created", content: "log(Title: #{@log.title} Active: #{@log.active})", user_id: current_user.id)
            redirect_to @log
        else
            render 'new'
        end
    ...
    def update
        ...
        if @log.update(params_log)
            @event = Event.create(action: "updated", content: "log(Title: #{@old.title} Active: #{@old.active}) to Title: #{@log.title} Active: #{@log.active}", user_id: current_user.id)
